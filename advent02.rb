s = %q(Game 1: 1 green, 1 blue, 1 red; 1 green, 8 red, 7 blue; 6 blue, 10 red; 4 red, 9 blue, 2 green; 1 green, 3 blue; 4 red, 1 green, 10 blue
...
results = s.split("\n").map do |game|
  ...
  round_results = rounds_str.split("; ").map do |round_str|
    round_str.scan(/(\d+)\s(\w+)/).map(&:reverse).to_h.map do |color, amount|
      amount.to_i <= maxes[color]
    end
  end
